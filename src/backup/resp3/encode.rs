...
use crate::{
  ...
  types::{RedisProtocolError, RedisProtocolErrorKind, CRLF},
  utils,
};
use alloc::vec::Vec;
use bytes::BytesMut;
...
///
/// This interface is stateful due to the fact that the caller almost always needs to maintain some state outside of
/// these functions. As a result it's up to the caller to manage the starting, middle, and ending state of data
/// streams in order to call the correct functions in this module.
///
/// Examples:
///
/// ```rust
