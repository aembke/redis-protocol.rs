use crate::{
  ...
  decode_mut::{frame::Resp2IndexFrame, utils::range_to_bytes},
  error::*,
  resp2::{
    decode::{isize_to_usize, NULL_LEN},
    types::{
      Frame as Resp2Frame,
      FrameKind,
      PartialFrame,
...
fn d_read_to_crlf_count(input: (&[u8], usize)) -> DResult<usize> {
  decode_log!(input.0, _input, "Counting to CRLF. Remaining: {:?}", _input);
  let (input_bytes, data) = nom_terminated(nom_take_until(CRLF.as_bytes()), nom_take(2_usize))(input.0)?;
...
fn d_parse_simplestring(input: (&[u8], usize)) -> DResult<PartialFrame> {
  let offset = input.1;
  let ((input, next_offset), len) = d_read_to_crlf_count(input)?;
  Ok((
    (input, next_offset),
    PartialFrame::SimpleString(offset .. (offset + len)),
  ))
}

fn d_parse_integer(input: (&[u8], usize)) -> DResult<PartialFrame> {
  let ((input, next_offset), data) = d_read_to_crlf_take(input)?;
  let parsed = etry!(to_i64(&data));
  Ok(((input, next_offset), PartialFrame::Integer(parsed)))
}
...
fn d_parse_null(input: (&[u8], usize)) -> DResult<PartialFrame> {
  Ok((input, PartialFrame::Null))
}

fn d_parse_error(input: (&[u8], usize)) -> DResult<PartialFrame> {
  let offset = input.1;
  let ((input, next_offset), len) = d_read_to_crlf_count(input)?;
  Ok(((input, next_offset), PartialFrame::Error(offset .. (offset + len))))
}

fn d_parse_bulkstring(input: (&[u8], usize), len: usize) -> DResult<PartialFrame> {
  let offset = input.1;
  let (input, data) = nom_terminated(nom_take(len), nom_take(2_usize))(input.0)?;
  Ok((
    (input, offset + len + 2),
    PartialFrame::BulkString(offset .. (offset + data.len())),
  ))
}

fn d_parse_bulkstring_or_null(input: (&[u8], usize)) -> DResult<PartialFrame> {
...
fn d_parse_array_frames(input: (&[u8], usize), len: usize) -> DResult<Vec<PartialFrame>> {
  decode_log!(
    input.0,
    _input,
...
fn d_parse_array(input: (&[u8], usize)) -> DResult<PartialFrame> {
  let ((input, offset), len) = d_read_prefix_len(input)?;
  ...
  if len == NULL_LEN {
    ...
    Ok(((input, offset), PartialFrame::Array(frames)))
  }
}

fn d_parse_frame(input: (&[u8], usize)) -> DResult<PartialFrame> {
...
/// Attempt to parse the contents of `buf`, returning the first valid frame, the number of bytes consumed, and the
/// frozen consumed bytes.
///
/// If the byte slice contains an incomplete frame then `None` is returned.
///
/// Unlike `decode` this function works on a mutable `BytesMut` buffer in order to parse frames without copying the
/// inner buffer contents and will automatically split off the consumed bytes before returning. If an error or `None`
/// is returned the buffer will not be modified.
